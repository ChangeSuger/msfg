...
const WIDTH = 100, HEIGHT = 30, PADDING = 100

export function exportStruct(graphStruct) {
  let nodeList = []
  let linkList = []
  let idMap = {}
...
export function importStruct(graphStruct, showType = 'edit', h = 900, w = 1200) {
  let nodeList = []
  let linkList = []
  let idMap = []
  let count = 0
...
export function renderStructColor(struct, showType = 'check') {
  ...
  for (let system of struct) {
    ...
    for (let node of system.data.nodes) {
      if (node.type === 'fault-node'||node.type === 'test-node') {
        if (showType === 'check') {
          node.properties.typeColor = getColorRGB_check(
            node.properties.collision,
            node.properties.fuzzible,
            node.properties.detectable,
            node.properties.typeColor
          )
          console.log("ate new color", node.properties.typeColor)
        } else if (showType === 'analyse') {
          node.properties.typeColor = getColorRGB_analyse(
            node.properties.state,
            node.properties.fuzzy_state
          )
          console.log("ate new color", node.properties.typeColor)
        }
      }
...
export function getColorRGB_check(collision, fuzzible, detectable, typeColor) {
  if (collision) {
    return "rgb(255, 0, 0)"
  } else if (fuzzible) {
    return "rgb(0, 0, 255)"
...
export function getColorRGB_analyse(state, fuzzy_state, fuzzy_ratio) {
  if (fuzzy_state == null) {
    fuzzy_state = 0
  }
  if (fuzzy_ratio == null) {
...
export function getColorRGB_check_sub(collision, fuzzible, detectable, typeColor) {
  if (collision) {
...
export function getColorRGB_analyse_sub(state, fuzzy_state, fuzzy_ratio) {
  if (fuzzy_state == null) {
    fuzzy_state = 0
  }
  if (fuzzy_ratio == null) {
